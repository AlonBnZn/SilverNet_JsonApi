...
{
    public class When_Calling_Update_With_Invalid_IdNumber : UserControllerSpecificationBase
    {
        ...
        public void Should_Not_Update_User()
        {
            ...
            user.IdNumber.Should().Be(_user.IdNumber);

            _response.Data.Should().BeNull();

            _response.Errors.Should().NotBeNull();

            _response.Errors[0].Detail.Should().Be("Invalid idNumber format.");
        }
    }
}
