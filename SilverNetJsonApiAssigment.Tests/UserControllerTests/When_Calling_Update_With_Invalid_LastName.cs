...
{
    public class When_Calling_Update_With_Invalid_LastName : UserControllerSpecificationBase
    {
        ...
        public void Should_Not_Update_User()
        {
            ...
            user.LastName.Should().Be(_user.LastName);

            _response.Data.Should().BeNull();

            _response.Errors.Should().NotBeNull();

            _response.Errors[0].Detail.Should().Be("lastName cannot exceed 10 characters.");
        }
    }
}
