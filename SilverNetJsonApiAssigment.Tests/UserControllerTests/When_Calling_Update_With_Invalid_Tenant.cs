...
{
    public class When_Calling_Update_With_Invalid_Tenant : UserControllerSpecificationBase
    {
        ...
        {
            _response = SendJsonApiRequest<UserResource>(HttpMethod.Patch, $"/api/v1/tenants/{99}/users/{_user.Id}", _request);
        }
        ...
        public void Should_Not_Update_User()
        {
            ...
            user.FirstName.Should().Be(_user.FirstName);

            _response.Data.Should().BeNull();

            _response.Errors.Should().NotBeNull();

            _response.Errors[0].Detail.Should().Be("Error updating user");
        }
    }
}
